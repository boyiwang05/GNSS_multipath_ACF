...
L_CA=length(c);
m=1;k=1;%BOC modulation settings
Rc=k*1.023e6;% code rate
Tc=1/Rc;% chip width
f_sample=500e6;
T_sample=1/f_sample;
Tp=1e-3-T_sample;
...
[S_CA_receiver_multipath,t]=yt_BPSK_function(c,t_begin-delay_t,t_end-delay_t,Tc,f_sample);
...
%     IL_up=sum(RecSigI_up.*s_L)/N_zong;QL_up=sum(RecSigQ_up.*s_L)/N_zong;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%             
     R_BPSK_like_multipath_added(k)=sqrt(IP_multipath_added^2+QP_multipath_added^2);%
     R_BPSK_like_multipath(k)=sqrt(IP_multipath^2+QP_multipath^2);
     R_BPSK_like_LOS(k)=sqrt(IP_LOS^2+QP_LOS^2);
     
     Discriminator_out(k)=((IE^2+QE^2)-(IL^2+QL^2))/( (IE^2+QE^2)+(IL^2+QL^2) );
         
    temp_string=['Running ' num2str(ceil(k/N_tao*10000)/100) '%'];
